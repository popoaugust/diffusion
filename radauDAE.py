...
# -*- coding: utf-8 -*-
"""
Created on Wed May 31 11:22:39 2023
            ...
            while not converged:
                ...
                if LU_real is None or LU_complex is None:
                  ...
                  try:
                    ...
                    _A_real = self.hscale @ (MU_REAL    / h * self.mass_matrix - J)
                    raise Exception(
                          f"MU_REAL = {MU_REAL}\n"
                          f"h = {h}\n"
                          f"hscale: min = {self.hscale.min()}, max = {self.hscale.max()}, mean = {self.hscale.mean()}"
                          f"mass: min = {self.mass_matrix.min()}, max = {self.mass_matrix.max()}, mean = {self.mass_matrix.mean()}\n"
                          f"J: min = {J.min()}, max = {J.max()}, mean = {J.mean()}\n"
                          f"A: min = {_A_real.min()}, max = {_A_real.max()}, mean = {_A_real.mean()}\n"
                          )
